...
**异世界人格类型量表** - 基于荣格心理类型理论、融合提瓦特元素体系的MBTI人格测试系统
...
| 数据存储 | SQLite + JSON文件 | 用户数据使用SQLite，静态数据使用JSON |
...
│   ├── admin.js                  # 管理后台接口
│   ├── ai.js                     # AI模型列表接口
│   ├── ai-analyze.js             # AI分析接口：大模型调用（SSE流式）
│   ├── data.js                   # 数据接口：题库、角色数据
│   ├── invite.js                 # 邀请码接口
│   ├── result.js                 # 结果接口：测试结果存储
│   ├── statistics.js             # 统计接口：数据统计
│   ├── user.js                   # 用户接口：登录、注册、进度保存
│   └── verification.js           # 验证问答接口
│
├── services/                     # 后端服务
...
│   ├── data.js                   # 数据服务：数据读取封装
│   ├── invite-code.js            # 邀请码服务
│   ├── json-db.js                # JSON数据库：文件读写
│   ├── result.js                 # 结果服务：结果存储
│   ├── sqlite-db.js              # SQLite数据库：用户数据、会话管理
│   └── verification.js           # 验证问答服务
│
├── scripts/                      # 工具脚本
│   ├── generate_quiz.js          # 题库生成脚本
│   └── generate_meme_quiz.js     # 梗题库生成脚本
│
└── assets/                       # 静态资源
    ├── audio/                    # 音频资源
    │   ├── bgm.mp3               # 背景音乐
...
- **分享卡片**：生成可分享的结果图片
- **用户系统**：登录注册，保存测试历史（支持SQLite持久化）
- **答题进度保存**：自动保存答题进度到服务器，支持中途退出继续
- **邀请码系统**：限制注册，防止滥用，可管理邀请码剩余次数
- **验证问答**：注册时自动验证，防止机器注册
- **MALLM多智能体讨论**：多个AI角色在线讨论，提供多维度的MBTI分析视角
- **管理后台**：查看统计数据、用户管理、邀请码管理
