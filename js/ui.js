...
const UI = {
    ...
    renderAboutTest: function () {
        ...
        this.aboutTestData.items.forEach(item => {
            // 首页只显示简化的娱乐性质说明
            if (item.startsWith('【娱乐性质】')) {
                html += `<li>【娱乐性质】本测试仅供娱乐参考，结果不代表专业心理评估。如有心理困扰，请寻求专业帮助。</li>`;
            } else {
                html += `<li>${item}</li>`;
            }
        });
    ...
    handleClick: function (e) {
        ...
        } else if (target.closest('[data-action="show-qrcode-modal"]')) {
     ...
     * 快速检查是否有进度（不加载完整数据）
     * 
     * @returns {Promise<Object>} { hasNormalProgress, normalProgressMeta, hasAIProgress, aiProgressMeta }
     */
    checkProgressQuick: async function () {
        try {
            const response = await fetch('/api/user/progress/check');
            const data = await response.json();
            return data;
        } catch (e) {
            console.error('[进度检查] 快速检查失败:', e);
            return { hasNormalProgress: false, hasAIProgress: false };
        }
    },
     ...
     * 
     * @param {Object} progressMeta - 进度元数据（不需要完整数据）
     * @param {string} type - 类型 'normal' 或 'ai'
     */
    showProgressPrompt: function (progressMeta, type) {
        const modal = document.getElementById('gender-modal');
        if (!modal) return;

        const savedTime = progressMeta.savedAt ? new Date(progressMeta.savedAt).toLocaleString('zh-CN') : '未知';
        let infoHtml = '';

        if (type === 'normal') {
            const answeredCount = progressMeta.answeredCount || 0;
            const questionSet = progressMeta.questionSet || '未知题库';
            infoHtml = `
                您在 <strong>${savedTime}</strong> 有一个未完成的测试<br>
                题库: <strong>${questionSet}</strong><br>
                已完成 <strong>${answeredCount}</strong> 题
            `;
        } else if (type === 'ai') {
            const answeredCount = progressMeta.answeredCount || 0;
            const totalCount = progressMeta.totalCount || 10;
            infoHtml = `
    ...
    continueProgress: async function () {
        ...
        if (!progressType) {
            console.error('[continueProgress] 未找到 progressType');
            return;
        }

        console.log('[continueProgress] 继续答题, 类型:', progressType);
        ...
        if (progressType === 'normal') {
            ...
            // 显示加载状态
            this.showLoading(true, '正在恢复答题进度...');
            ...
            // 显示加载状态
            this.showLoading(true, '正在恢复AI答题进度...');
            ...
            try {
                ...
                if (data.success && data.hasProgress && data.data) {
                    if (window.AIAnalysis && typeof window.AIAnalysis.resumeProgress === 'function') {
                        await window.AIAnalysis.resumeProgress(data.data);
    ...
    renderResultDetail: function (result) {
        ...
        if (typeof ResultRenderer !== 'undefined') {
            ...
            if (result.isAIAnalysis && (result.aiAnalysis || result.analysis)) {
                console.log('[渲染结果] 渲染 AI分析模块');
                // 传入整个 result 对象，因为 confidence, detailedEvidence 等字段在顶层
                this.renderAIAnalysis(result, result.type);
            }
        } else {
     ...
     */
    renderAIAnalysis: function (result, mbtiType) {
        ...
        // AI分析数据可能在顶层，也可能在 aiAnalysis 字段中
        const aiAnalysis = result.aiAnalysis || result;
        if (!aiAnalysis.analysis && !aiAnalysis.analysisSummary) return;
        ...
        // 分析总结
        if (aiAnalysis.analysisSummary || aiAnalysis.analysis) {
            html += '<div class="obc-tmpl-part">';
            html += '<h2 class="wiki-h2">分析总结</h2>';
            html += '<table><tbody>';
            html += '<tr><td colspan="2" class="obc-tmpl__rich-text">' + TermRenderer.render(aiAnalysis.analysisSummary || aiAnalysis.analysis) + '</td></tr>';
     ...
     * @returns {Object|null} 进度数据或null
     */
    checkAIProgress: async function () {
        try {
            const response = await fetch('/api/user/progress/ai');
            const data = await response.json();
            if (data.success && data.hasProgress && data.data) {
    ...
    startAITestAfterGender: function () {
        // 直接启动AI深度分析测试，由confirmGenderAndStart处理
    },
     ...
     */
    startAITestQuick: async function (count) {
        this.resetAITipsScrollCheck();
    ...
    checkHistoryResult: function () {
        ...
        try {
            ...
            if (history.length > 0) {
                ...
                if (historyTime) {
                    historyTime.textContent = '查看历史测试记录';
                }
            }
        } catch (e) {
    ...
    showHistoryModal: async function() {
        ...
        try {
            ...
            if (results.length === 0) {
                body.innerHTML = '<div class="export-empty">完成测试后即可导出答卷</div>';
                return;
            }
    ...
    showQrcodeModal: function () {
        ...
        const closeBtn = document.getElementById('qrcode-modal-close');
